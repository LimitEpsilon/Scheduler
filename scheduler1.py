...
class Scheduler:
	...
	def __init__(self):
		...
		h = list(map(int, input("이번 달에 휴일은 무슨 날짜에 있습니까? (콤마 없이): ").strip().split()))
		v = []
		for ppl in range(self.no_people):
			v.append(
				list(
					map(int,
									input(str(ppl) + "번째 사람은 무슨 날짜에 휴가를 갑니까? (콤마 없이): ").strip().split())))
		for date in h:
	...
	def test(self, l):
		...
		if l % 2 == 0:
			return (schedule[l - 1] != schedule[l]) and (
				schedule[l - 2] != schedule[l]) and (schedule[l - 3] != schedule[l]) and (
